...
#define PARENT_PRIORITY			1" > $home_dir/param.h

if [ $(echo "$check_rate >= 1" | bc) -eq 1 ]; then
  echo "#define CHECK_RATE		($check_rate)" >> $home_dir/param.h
elif [ $(echo "$check_rate < 1" | bc) -eq 1 ]; then
  check_rate_=$(echo $check_rate | awk '{print $1*10}')
  echo "#define CHECK_RATE		($check_rate_)" >> $home_dir/param.h
fi

echo "#define PC_ON_TIME_R 	(RTIMER_ARCH_SECOND / 160)
